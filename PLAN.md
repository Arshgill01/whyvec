...
| R4 | LLVM optimization-causality pack | verifying | Public `explain-opt` and `replay-opt` compose typed variants, recorded pipeline replay, structural loop identity, confirmation runs, finite search, tool fingerprints, structured records, immutable reports, and semantic reproduction; retained integrated ambiguity reports remain |
