...
int add_vectors_guarded(int *output, const int *input, const int *count);
...
static void guarded_adapter(int *output, const int *input, const int *count) {
  if (add_vectors_guarded(output, input, count) != 1)
    abort();
}
...
int main(void) {
  ...
  guarded_adapter(output, input, &count);
  ...
  for (int sample = 0; sample < SAMPLES; ++sample) {
    if ((sample & 1) == 0) {
      ...
      guarded[sample] = measure(guarded_adapter, output, input, &count);
    } else {
      guarded[sample] = measure(guarded_adapter, output, input, &count);
      original[sample] = measure(add_vectors_original, output, input, &count);
    }
